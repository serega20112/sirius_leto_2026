...
5. Для нового пайплайна вовлеченности на MediaPipe установите AI-дополнения, если их нет в окружении:
...
- Распознавание теперь строится на `FaceNet512`-эмбеддингах: при регистрации через форму (`/register`) галерея эмбеддингов пересобирается автоматически.
- Для уменьшения ложных совпадений используйте 3 фото на человека с разным углом лица и хорошим освещением.

Оценка вовлеченности
- Вовлеченность рассчитывается через `MediaPipe Face Mesh + Pose`, а не через одиночный pose-кадр.
- Основные пороги можно калибровать через `.env`:

```
FACE_DISTANCE_THRESHOLD=0.30
FACE_DISTANCE_MARGIN=0.04
FACE_MIN_STABLE_VOTES=3
