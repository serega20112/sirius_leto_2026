...
class MonitorService:
    ...
    def _fallback_image_generator(self):
        ...
        while True:
            frame = img.copy()
            if self.track_attendance_use_case and getattr(
                self.track_attendance_use_case, "person_detector", None
            ):
                try:
                    ...
                    if (
                        ...
                        print(
                            "[Monitor] unexpected tracking_result type:",
            ...
            try:
                ...
                while True:
                    ...
                    if not ret or frame is None:
                        ...
                        if retry > 10:
                            print(
                                "[Monitor] camera read failed repeatedly, switching to fallback image"
                            )
                            yield from self._fallback_image_generator()
                            return
                        time.sleep(0.05)
                    ...
                    if self.track_attendance_use_case and getattr(
                        ...
                        try:
                            tracking_result = self.track_attendance_use_case.execute(
                                frame
                            )
                            if (
                                isinstance(tracking_result, dict)
